...
\begin{table}[ht]
\centering
\caption{Exactitud multiclase (29 familias) según el subconjunto de características. Validación cruzada de 5 particiones sobre 29.029 archivos. Azar $= 0{,}034$. Este experimento se realizó antes de incorporar BLACKBASTA al conjunto, de modo que su base son 29 familias; los Experimentos 2b y 2c posteriores se miden sobre las 30.}
...
El procedimiento identificó marcas estructurales en \textbf{28 de las 30 familias}. En quince casos se trata de firmas binarias en el contenido, varias de ellas legibles como texto: WANNACRY escribe \texttt{57 41 4E 41 43 52 59 21} al inicio de cada archivo, secuencia que corresponde a la cadena \texttt{WANACRY!}; CUBA inserta \texttt{FIDEL.CA} en los primeros veinte bytes; PHOBOS añade \texttt{LOCK96} al final; BADRABBIT escribe la palabra \texttt{encrypted}, codificada en UTF-16, al final de cada archivo. Otras familias emplean firmas extensas no textuales: CERBER, LOCKBIT, RANSOMEXX y TESLACRYPT presentan bloques constantes de 64 bytes.
...
Dos familias ---NOTPETYA y SUNCRYPT--- no presentan marca alguna. SUNCRYPT asigna a cada archivo una extensión distinta, un valor hexadecimal de 64 caracteres, de modo que no existe extensión común que registrar. El caso de NOTPETYA coincide con lo documentado por Davies et al.~\cite{davies2023majority}, quienes observan que esta familia no modifica la extensión de los archivos que ataca y que, por ese motivo, evade las pruebas basadas en nombre y extensión. La convergencia entre ambos trabajos, obtenida por vías metodológicas distintas, refuerza la validez del procedimiento.

\subsection{Resultados y ablación}

Un resultado agregado del 93,2\% de exactitud podría atribuirse por completo a la extensión del archivo, que es también el mecanismo que emplean las herramientas basadas en reglas. Para separar ambas contribuciones se repitió la evaluación en tres modalidades (Tabla~\ref{tab:exp2b}).

\begin{table}[ht]
\centering
\caption{Ablación del Experimento 2b: contribución de cada tipo de marca (1.500 archivos, 30 familias, \textit{leave-one-out}; media $\pm$ desvío sobre diez semillas de muestreo, con el criterio de detección declarado en 0,90)}
\label{tab:exp2b}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Modalidad} & \textbf{Cobertura} & \textbf{Exactitud global} & \textbf{Exactitud donde aplica} \\
\hline
Solo extensión del archivo & 86,7\% & 0,867 $\pm$ 0,000 & 1,000 $\pm$ 0,000 \\
Solo firmas binarias & 57,2\% & 0,563 $\pm$ 0,002 & 0,984 $\pm$ 0,003 \\
Combinado & 94,1\% & 0,932 $\pm$ 0,001 & 0,991 $\pm$ 0,002 \\
\hline
\end{tabular}
\end{table}
...
Las firmas binarias, en cambio, constituyen evidencia de contenido: son marcadores que el propio programa escribe en el archivo para reconocer posteriormente lo que ya ha cifrado, y por lo tanto resultan más estables entre campañas que la extensión. Alcanzan un 98,4\% de acierto, pero solo cubren el 57,2\% de los archivos: la regla exige que la firma se repita en al menos el 90\% de los archivos de la familia, de modo que las familias cuyo marcador varía entre archivos quedan fuera. Esta limitación de cobertura es la que motiva el experimento siguiente.
...
\hline
\textbf{Posicional + Random Forest (final)} & \textbf{15.000} & \textbf{0,912 $\pm$ 0,002} & \textbf{0,911 $\pm$ 0,001} \\
\hline
\end{tabular}
\end{table}

La mejor configuración alcanza una \textbf{exactitud de 0,912 $\pm$ 0,002 y un macro-F1 de 0,911 $\pm$ 0,001 sobre 30 familias, con cobertura total y utilizando exclusivamente el contenido del archivo} (media y desvío sobre diez semillas de muestreo). Los hiperparámetros seleccionados fueron 300 árboles, profundidad máxima 20, mínimo de 2 muestras por hoja y 30\% de características por división.
...
El resultado es marcadamente bimodal: \textbf{23 de las 30 familias alcanzan un F1 igual o superior a 0,98}, mientras que seis quedan por debajo de 0,75. La correspondencia con el Experimento 2b es notable y opera como validación cruzada entre ambos:
...
\item De las \textbf{once familias que solo poseían extensión propia} ---es decir, sin marca detectable en el contenido mediante coincidencia exacta---, siete alcanzan igualmente valores superiores a 0,98. El clasificador extrae de los bytes información que la regla exacta no capturaba. Las cuatro restantes ---JIGSAW, DARKSIDE, CRYPTOLOCKER y WASTEDLOCKER--- son precisamente las que el Experimento 2c no resuelve: al no emplear el nombre del archivo, el clasificador se queda sin la única marca que esas familias dejan.
\item De las \textbf{dos familias sin marca alguna}, ninguna supera el umbral: SUNCRYPT alcanza 0,745 $\pm$ 0,016 y NOTPETYA 0,394 $\pm$ 0,023. BADRABBIT, en cambio, pasa de un F1 de 0,15 en el experimento estadístico a \textbf{0,978 $\pm$ 0,002}, quedando prácticamente resuelta.
\end{itemize}

Las seis familias con menor rendimiento ---SUNCRYPT (0,745 $\pm$ 0,016), WASTEDLOCKER (0,628 $\pm$ 0,013), CRYPTOLOCKER (0,605 $\pm$ 0,016), DARKSIDE (0,603 $\pm$ 0,011), JIGSAW (0,439 $\pm$ 0,014) y NOTPETYA (0,394 $\pm$ 0,023)--- forman un grupo de confusión mutua. DARKSIDE presenta un patrón revelador: precisión de 0,45 frente a un recall de 0,90, lo que indica que actúa como clase de destino por defecto para los archivos que el modelo no logra atribuir. Estas seis familias son, presumiblemente, las que cifran sin añadir estructura al archivo, y constituyen el residuo genuino de la indistinguibilidad estadística planteada en el Experimento 2.

\subsection{Limitación}
\label{subsec:exp2c_limitacion}

El conjunto NapierOne representa cada familia mediante una única campaña, obtenida ejecutando una muestra concreta del ransomware. En consecuencia, el valor de 0,912 mide la capacidad de identificar \textit{esa campaña}, y no puede afirmarse que se extienda a campañas futuras de la misma familia, que podrían modificar sus cabeceras o colas. Esta limitación es análoga a la observada en el corpus de notas de rescate, donde la baja variabilidad de plantillas produce el mismo efecto (Sección~\ref{subsec:res_variabilidad}), y su evaluación requeriría un conjunto de datos con múltiples campañas por familia, no disponible en la actualidad.
...
La conclusión del frente de archivos cifrados queda, por tanto, acotada con precisión: la indistinguibilidad estadística es real, pero se limita a \textbf{seis de las treinta familias evaluadas}: dos que no dejan marca alguna y cuatro que solo marcan el nombre del archivo, sin añadir estructura reconocible a su contenido. Para las veinticuatro restantes la información de familia está presente en el contenido del archivo y es extraíble automáticamente con una exactitud de 0,912 $\pm$ 0,002.
...
Exp. 2b: Firmas binarias (donde aplican) & 30 & 98,4\% acc. (57\% cobert.) & Coincidencia exacta \\
Exp. 2c: Aprendizaje sobre bytes & 30 & \textbf{91,2\% acc. / 0,911 macro-F1} & Random Forest (posicional) \\
...
\hline
ID Ransomware (notas) & 22 & 71,93\% (41/57 notas) & Reglas manuales \\
ID Ransomware (archivos) & 30 & 66,67\% (30,0\% al renombrar) & Reglas y \textit{magic bytes} \\
CryptoSheriff (archivos) & 30 & 16,67\% & Reglas manuales \\
\hline
\end{tabular}
}
\end{table}
...
\item \textbf{Identificación de familia por el contenido del archivo cifrado} (91,2\%), aplicable a las veinticuatro familias en cuyo contenido persiste información de familia.

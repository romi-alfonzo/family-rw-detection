...
def main():
    ...
    features = []
    labels = []
    if os.path.exists(features_file1) and os.path.exists(features_file2):
        with open(features_file1, 'r', newline='', encoding='utf-8') as csvfile1:
            with open(features_file2, 'r', newline='', encoding='utf-8') as csvfile2:
                ...
                for row1, row2 in zip(reader1, reader2):
                    features.append([float(row1[0]), float(row1[1]), float(row2[0])])
    ...
    # print file names
    print(f'Features: {features_file1} and {features_file2}')
